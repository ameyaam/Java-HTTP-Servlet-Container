...
public class TestHarness {	
	static class Handler extends DefaultHandler {
		static final int SERVLET_MAPPING = 5;
		static final int SERVLET_MAPPING_NAME = 51;
		static final int SERVLET_URL_PATTERN = 52;
		public void startElement(String uri, String localName, String qName, Attributes attributes) {
			if (qName.compareTo("servlet-name") == 0) {
				if(m_state == SERVLET_MAPPING)
				{
					m_state = SERVLET_MAPPING_NAME;
				}
				else
				{
					m_state = 1;
				...
				m_state = SERVLET_URL_PATTERN;
			} else if (qName.compareTo("servlet-mapping") == 0) {
				m_state = SERVLET_MAPPING;
			}
		...
		public void characters(char[] ch, int start, int length) {
			...
			if (m_state == 1) {
				...
				m_paramName = null;
				m_state = 0;
			} else if (m_state == SERVLET_URL_PATTERN)
			{
				m_servletUrls.put(m_servlet_name_mine, value);
				m_paramName = null;
				m_state = 0;
			} else if (m_state == SERVLET_MAPPING_NAME) {
				if(value != null) {
					m_servlet_name_mine = value;
				}
				m_state = SERVLET_MAPPING;
			}
		...
		private String m_servlet_name_mine;
	...
	static HashMap<String,String> createServerUrlMap(Handler h)
	{
		return h.m_servletUrls;
	}
	...
	static FakeContext createContext(Handler h) {
	...
	static HashMap<String,HttpServlet> createServlets(Handler h, FakeContext fc) throws Exception {
